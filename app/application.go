...
func (app *Application) Query(reqQuery abci.RequestQuery) (abci.ResponseQuery) {
	...
	case "/newsample":
		{
			...
			// Verify identity of the sample. Sample must have been generated by a consensus node.
			identity, ok := app.GetAccessList().Identities[storageSample.Identity]
			if !ok || identity.AccessLevel != conf.Consensus {
				return abci.ResponseQuery{Code: uint32(types.CodeType_Unauthorized), Log: "Identity unauthorized"}
			}

			// Check if public key exists and if message is signed.
			if pk, err := crypto.LoadPublicKey(conf.AppConfig().BasePath + conf.AppConfig().PublicKeys + identity.PublicKey); err != nil {
				return abci.ResponseQuery{Code: uint32(types.CodeType_BCFSInvalidSignature), Log: "Could not locate public key"}
			} else if !signedStruct.Verify(pk) {
				return abci.ResponseQuery{Code: uint32(types.CodeType_BCFSInvalidSignature), Log: "Could not verify signature"}
			}
			signedStruct.VerifyChallenge()

			// Check if this sample is already stored.
			if _, err := os.Lstat(conf.AppConfig().StorageSamples + storageSample.Cid); err == nil {
				return abci.ResponseQuery{Code: uint32(types.CodeType_InternalError), Log: "Sample for this file is already stored."}
			}
	...
	default:
		{
			resQuery.Code = uint32(types.CodeType_BCFSInvalidInput)
